...
alter table paquete_de_viaje add column ctd_dias int;

#1
select min(ctd_dias) from paquete_de_viaje;
#2
select max(ctd_dias) from paquete_de_viaje;
#3
select avg(cost) from paquete_de_viaje where ctd_dias=3;
#4
select sum(cdt_de_personas) from reservas where fecha_de_inicio like "2024-07-%";
#5
select distinct lugar from paquete_de_viaje;
#6
select sum(cdt_de_excursiones) from paquete_de_viaje where lugar like "bariloche";
#7
select count(*) from reservas where paquete_de_viaje_codigo=1;
#8
select distinct paquete_de_viaje_codigo from reservas;
#9
update paquete_de_viaje set cost=cost+((cost*10)/100);
#10
select count(codigo),alojamientos_codigo from paquete_de_viaje group by alojamientos_codigo;
#11
update paquete_de_viaje set ctd_dias=ctd_dias+1 where lugar="Salta";
#12
select
